...
class OrderHistory extends Component {
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            ...
            >
              <Text
                style={[
                  styles.salesHistoryDataFormat,
                  { color: "#454974", fontSize: 13, fontWeight: "200" }
                ]}
              >
                {item.id}
              </Text>
              ...
              >{`$${(item.amount - item.tax).toFixed(2)}`}</Text>
              <Text
                style={[
                  styles.salesHistoryDataFormat,
                  { color: "#454974", fontSize: 13, fontWeight: "200" }
              ...
              >{`$${
                item.discount ? item.discount.toFixed(2) : item.discount
              }`}</Text>
              <Text
                style={[
                  styles.salesHistoryDataFormat,
              ...
              >{`$${item.tax ? item.tax.toFixed(2) : item.tax}`}</Text>
              <Text
                style={[
                  styles.salesHistoryDataFormat,
                  { color: "#454974", fontSize: 13, fontWeight: "200" }
                ]}
              >{`$${item.amount ? item.amount.toFixed(2) : item.amount}`}</Text>
            </View>
          )}
        />
      );
      ...
      return (
        ...
        >
          ...
          <Text style={{ fontSize: 20, marginTop: 30, fontWeight: "bold" }}>
            After Selling an Item you will be able to track it here
          </Text>
        </View>
      );
    }
  ...
  render() {
    ...
    // Actual Sales Data
    const salesHistory = this.props.store.BusinessStore.filterCompletedTransactions(
      this.state.salesHistoryTimeFrame
    );
    const productHistory = this.props.store.BusinessStore.filterCompletedTransactions(
      this.state.productHistoryTimeFrame
    );
    return (
      <ScrollView
          ...
          <View style={styles.sales}>
            <View style={styles.mainCardHeader}>
              ...
              <Dropdown
                ...
                textColor={"black"}
                onChangeText={item =>
                  this.setState({ salesHistoryTimeFrame: item })
                }
                data={sortBy}
              />
            </View>
          ...
          <View style={styles.products}>
            <View style={styles.mainCardHeader}>
              ...
              <Dropdown
                ...
                textColor={"black"}
                onChangeText={item =>
                  this.setState({ productHistoryTimeFrame: item })
                }
                data={sortBy}
              />
            </View>
