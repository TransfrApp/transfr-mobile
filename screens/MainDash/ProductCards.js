...
class ProductCards extends Component {
  ...
  handleSearchCardClick(index) {
    const searchProducts = this.props.store.BusinessStore.business
      .searchProductList;
    const business = this.props.store.BusinessStore;
    ...
    if (search.length > 0) {
      return search.map((item, index) => {
        return (
          <TouchableOpacity
            key={index}
            onPress={() => this.handleSearchCardClick(index)}
            style={list ? styles.wideCard : styles.card}
          >
            <Image
              style={list ? styles.wideCardImage : styles.cardImage}
              source={{ uri: item.image }}
            />
            <Text style={list ? styles.prodNameWideCard : styles.prodName}>
              {item.name}
            </Text>
            <Text
              style={list ? styles.priceWideCard : styles.price}
      ...
      return products.map((item, index) => {
        return (
          <TouchableOpacity
            key={index}
            onPress={() => this.handleCardClick(products, index)}
            style={list ? styles.wideCard : styles.card}
          >
            <Image
              style={list ? styles.wideCardImage : styles.cardImage}
  ...
  render() {
    return (
      <View
        style={{ height: height * 0.85, width: width * 0.64, paddingLeft: 20 }}
      >
        <View
          style={{
            flexDirection: "row",
